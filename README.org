...
GitHub does not render the equations well.  Read the web version of this README:  [[https://raw.githack.com/JohnNehls/PyTorchExamples/main/README.html][LINK HERE]].

* Linear Regression
** Prediction equation
Linear Regression is used to predict a numerical value, $\hat{y}$, from observations, $X = \{x_0, x_1,\cdots,  x_{n-1}\}$ with the equation  $$\hat{y}(X) = \sum_{i=0}^{k-1} w_i f_i (X)$$  where the linear name (in linear regression)  comes from the fact that the equation is linear w.r.t. the weights, $w_i$.
The combination of function and data point $f_i (X)$ is called a *feature*. Features make up the /basis/ for the regression, thus should be unique.
...
The goal is to have the predictions, $\hat{y}(X)$, be accurate. This is done by having lots of data, $\{Y, X \}^i$ which now includes the observed values of the parameter we hope to predict, $y$, to compare to.
We do this by minimizing the loss,
$$loss = \frac{1}{N}\sum_{i=0}^{N-1}\left( y^i - \hat{y}(X^i) \right)^2$$,
where the only free parameters are the weights in $\hat{y}$. In the linear regression paradigm, the features are chosen  before the optimization. /footnote:/ In deep learning choosing the features is also part of the optimization process.
...
# Local Variables:
# mode: org
# eval: (add-hook 'after-save-hook 'jmn-org-export-html-on-save)
# end:
